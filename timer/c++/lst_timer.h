/*
 * sort_timer_lst是一个升序链表。其核心函数tick相当于一个心搏函数，它每隔一段固定的时间就执行一次，以检测并处理到期的任务。判断定时
 * 任务到期的依据是定时器的expire值小于当前的系统时间。从执行效率来看，添加定时器的时间复杂度是O(n)，删除定时器的时间复杂度是O(1)，执
 * 行定时任务的时间复杂度是O(1)
 *
 * Copy from Linux高性能服务器编程 11-2
 * Datetime: 2018/2/2 17:00
 *
 */
...
public:
    ...
    void add_timer(util_timer *timer)
    {
         ...
         * 否则就需要调用重载函数add_timer(util_timer* timer, util_timer* lst_head)，把它插入链表中合适的位置，
         * 以保证链表的升序特性
         */
...
private:
    ...
    void add_timer(util_timer *timer, util_timer *lst_head)
    {
        ...
        /*
         * 如果遍历完lst_head节点之后的部分链表，仍未找到超时时间大于目标定时器的超时时间的节点，则将目标定时器插入链表尾部，并把它
         * 设置为链表新的尾节点
         */
        if (!tmp)
        {
