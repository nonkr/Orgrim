...
int              g_nUsartfd         = -1;
static int       G_REQUEST_INTERVAL = 100;
...
void DecodeIMUData(const char *pData, size_t nSize)
{
    ...
    {
        printf("imu data size error, in size:%zu, structure size:%zu\n", nSize, sizeof(EvIMUReplyData));
        return;
    }
    else
    {
        EvIMUReplyData *pEvIMUReplyData = (EvIMUReplyData *) pData;
        unsigned short usDust           = ntohs(pEvIMUReplyData->usDust);
        printf("%d\n", usDust);
    }
}
...
//        OGM_PRINT_GREEN("]\n");
        ...
        usleep(1000 * G_REQUEST_INTERVAL);
    }
    pthread_exit((void *) 0);
}
...
//                        OGM_PRINT_ORANGE("]\n");
                        if (recv_len >= 5)
                        {
                            unsigned char cmd = (unsigned char) buf[3];
                            switch (cmd)
                            {
                                case 0x81:
...
int main(int argc, char **argv)
{
    if (argc > 1)
    {
        G_REQUEST_INTERVAL = atoi(argv[1]);
    }
