/*
 * Copy from https://github.com/ankushagarwal/nweb.git
 *
 */
...
//            buffer[i] = '*';
    ...
    if (!strncasecmp(&buffer[0], "GET /\0", 6)) /* convert no filename to index file */
...
int main(int argc, char **argv)
{
    ...
    if (!strncmp(argv[2], "/", 2) || !strncmp(argv[2], "/etc", 4) ||
        !strncmp(argv[2], "/bin", 4) || !strncmp(argv[2], "/lib", 4) ||
        !strncmp(argv[2], "/tmp", 4) || !strncmp(argv[2], "/usr", 4) ||
        !strncmp(argv[2], "/dev", 4) || !strncmp(argv[2], "/sbin", 5))
    {
    ...
    if (port <= 1024 || port > 60000)
        logger(ERROR, "Invalid port number (try 1025->60000)", argv[1], 0);
